...
function wordsList(str, subStr) {
  ...
  return new Set(str.split(" ").filter((item) => item.includes(subStr)));
}
let myLongStr =
...
//let myLongStr ='lorem Lorem". ipsum, dolor! sit? amet: consectetur; adipisicing elit. Dolores quas alias animi';
...
function getLocalDate(date, isSeconds, isISO) {
  ...
  let formSec;
  if (!isSeconds) formSec = `, ${hours}:${minutes}`;

  if (isSeconds) formSec = `, ${hours}:${minutes}:${seconds}`;

  if (isISO) formDate = `${year}-${month}-${day}`;

  return `${formDate}${formSec}`;
}
...
function getWeekDay(date) {
  ...
  return d.toLocaleDateString("ru", { weekday: "long" });
}
...
function getLocalDay(date) {
  ...
  let day = components[2];
  let d = new Date(year, month - 1, day);
  d = d.toLocaleDateString("ru", { weekday: "long" });
  switch (d) {
    case "понедельник":
      return 1;
      break;
